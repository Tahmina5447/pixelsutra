...
const ApplyCoupon = ({ setCouponDiscount, query = {}, product = [] }) => {
  const [couponCode, setCouponCode] = useState("");
  const { localStorageCartItems } = useContext(CreateContext);

  const url = `${server_url}/coupons/coupon-verify`;

  // const url = `https://latest-pixel-server-two.vercel.app/api/v1/coupons/coupon-verify`;
  ...
  const handleApplyCoupon = () => {
    ...
    if (query.from === "true") {
      ...
      cartProduct = localStorageCartItems.items;
    }
